...
export function renderGraph(graph2DDiv, node = null, selectNodeCallback = null, drillDownCallback = null, level = 2) {
    // Clear previous graph
    graph2DDiv.innerHTML = "";

    // Prepare nodes and links for d3 from layers and their relationships
    const layers = node._children || [];
...
export function create2DDefault(graph2DDiv, node = null, selectNodeCallback = null, drillDownCallback = null, level = 2) {
    // determine the “root” we’ll display
    let root;
    if (node) {
        console.log("Drilling into node:", node.id);
        root = node;
    } else {
        // subscribe once to layerNodes and build a fake GEAR root
        const nodes = get(layerNodes);
        root = {
            id: "_GEAR_",
            name: "GEAR",
            color: "#fff",
    ...
    // finally render
    renderGraph(graph2DDiv, root, selectNodeCallback, drillDownCallback);
}
